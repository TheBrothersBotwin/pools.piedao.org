<script>
  ...
  async function swap() {
    ...
    const gasPercentagePlus = BigNumber(quote.gas.toString())
      .multipliedBy(BigNumber(2.1))
      .toFixed(0);
    const transaction = {
      to: quote.to,
