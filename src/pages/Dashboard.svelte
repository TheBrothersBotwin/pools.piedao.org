...
<Meta
  metadata={{
    ...
    description:
      "An overview of the PieDAO's products, including Pies and PieVaults DEFI index. BCP, DEFI++, DEFI+L, DEFI+S, BTC++, USD++, YPIE.",
  }}
/>
...
<div class="content flex flex-col spl">
  ...
  <div class="w-99pc m-4">
    {#if piVaults.length}
      ...
      <div
        class="flex flex-col justify-around w-100pc content-center lg:flex-row hidden md:flex lg:flex"
      >
          ...
          <tbody>
            {#each piVaults as pie}
              <tr class="row-highlight">
                <td
                  class="pointer border border-gray-800 px-2 py-2 text-left min-w-180px"
                  on:click={() => (window.location.hash = `#/pie/${pie.address}`)}
                >
                  <a class="flex items-center px-2 py-2" href={`#/pie/${pie.address}`}>
                    <img class="inline icon ml-2 mr-2" src={pie.icon} alt={pie.symbol} />
                    <span class="md:block">{pie.symbol}</span>
                  </a>
                </td>
                <td
                  class="pointer border px-4 ml-8 py-2 font-thin text-center min-w-200px"
                  on:click={() => (window.location.hash = `#/pie/${pie.address}`)}
                >
                ...
                <td
                  class="border text-center w-12pc px-4 py-2"
                  on:click={() => (window.location.hash = `#/pie/${pie.address}`)}
                >
                  <Change value={pie.change} />
                </td>
                <td
                  class="pointer border px-4 ml-8 py-2 font-thin text-center"
                  on:click={() => (window.location.hash = `#/pie/${pie.address}`)}
                >
                  {pie.price}
                </td>
          ...
          <a class="mx-4 thinborderbottom" href={`#/pie/${pie.address}`}>
            <div class="flex items-center w-100pc py-4">
              ...
              <div class="flex flex-col justify-around max-w-55pc">
                ...
                <span class="text-sm font-thin opacity-40"
                  >{pie.description ? pie.description : 'Another great porfolio'}</span
                >
      ...
      <table class="breakdown-table table-auto w-full">
        ...
        <tbody>
          {#each pies as pie}
            <tr class="row-highlight">
              <td
                class="pointer border border-gray-800 px-2 py-2 text-left min-w-140px"
                on:click={() => (window.location.hash = `#/pie/${pie.address}`)}
              >
                <a class="flex items-center px-2 py-2" href={`#/pie/${pie.address}`}>
                  <img class="inline icon ml-2 mr-2" src={pie.icon} alt={pie.symbol} />
    ...
    {#if governingMining.length}
      ...
      <div
        class="flex flex-col justify-around w-99pc content-center lg:flex-row hidden md:flex lg:flex"
      >
        <table class="breakdown-table table-auto w-full">
          <thead>
            <tr>
            ...
            {#each governingMining as pie}
              <tr class="row-highlight">
                <td
                  class="pointer border border-gray-800 px-2 py-2 text-left min-w-180px"
                  on:click={() =>
                    (window.location.hash = pie.symbol == 'SLICE' ? '' : `#/pie/${pie.address}`)}
                >
                  <a
                    class="flex items-center px-2 py-2"
                    href={pie.symbol == 'SLICE' ? '#/slice' : `#/pie/${pie.address}`}
                  >
                    <img class="inline icon ml-2 mr-2" src={pie.icon} alt={pie.symbol} />
                    <span class="md:block">{pie.symbol}</span>
                  </a>
                ...
                <td
                  ...
                  on:click={() =>
                    (window.location.hash =
                      pie.symbol == 'SLICE' ? '#/slice' : `#/pie/${pie.address}`)}
                >
